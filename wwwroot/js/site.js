...
function HSL() {
    var color = `hsl(${document.getElementById('hslh').value}, ${document.getElementById('hsls').value}%, ${document.getElementById('hsll').value}%)`;
    c = w3color(color);
    document.getElementById('colorInputId').value = c.toHexString();
    convertColor('HSL');
    ...
    if (c.valid) {
        for (var space of spaces) {
            if (space != Elem) {
                ...
                if (space = "RGB") {
                    document.getElementById('rgbar').value = c.red;
                    document.getElementById('rgbag').value = c.green;
                    document.getElementById('rgbab').value = c.blue;
                }
                if (space = "HSL") {
...
function SetFocusedColor(color) {
    c = w3color(color);
    document.getElementById('colorInputId').value = c.toHexString();
...
function AddToPallet(fromColor) {
    ...
    // Unfortunately, the below converts hex to rgb.
